...
erDiagram
    ...
    ACTIVITY_REPORTS {
        string id PK "Report UUID"
        string activityId FK "Relasi ke ACTIVITIES.id"
        string submittedByUserId FK "Relasi ke USERS.id"
        string notes "Notulensi dan capaian hasil kegiatan"
        int actualAttendeesCount "Kehadiran riil warga"
        int budgetIncome "Realisasi kas / iuran masuk"
        int budgetSpent "Realisasi pengeluaran riil"
        string budgetNotes "Keterangan anggaran"
        string status "SUBMITTED | VERIFIED_RW | VERIFIED_KELURAHAN"
        string verifiedByRwName "Nama pengesah tingkat RW"
        string verifiedByAdminName "Nama pengesah tingkat Kelurahan"
        string qrVerificationCode "Kode token integritas dokumen"
        timestamp submittedAt "Waktu pelaporan"
    }
```
